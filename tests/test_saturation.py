...
def test_floor_zero_is_pure_mood():
    p = compile_mod.build_palette(RAW, fake_scheme(), saturation_floor=0.0)
    assert color.hsl(p["red"])[1] < 0.25  # stays muted at floor 0


def test_floor_lifts_hue_saturation():
    p = compile_mod.build_palette(RAW, fake_scheme(), saturation_floor=0.5)
...
def test_manifest_floor_used_when_no_override():
    m = fake_scheme(mood={"saturation_strength": 1.0, "bg_lightness": 0.07,
...
def test_compile_theme_returns_result_with_stats(tmp_path):
    ...
    result = compile_mod.compile_theme(tmp_path, "t", m, saturation_floor=0.5)
...
def test_build_palette_rejects_floor_above_one():
    with pytest.raises(ValueError, match=r"\[0.0, 1.0\]"):
        compile_mod.build_palette(RAW, fake_scheme(), saturation_floor=1.5)


def test_build_palette_rejects_negative_floor():
    with pytest.raises(ValueError, match=r"\[0.0, 1.0\]"):
        compile_mod.build_palette(RAW, fake_scheme(), saturation_floor=-0.1)
